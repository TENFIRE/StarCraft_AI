...
{
	strategies.push_back(Strategy(Races::Protoss, ProtossStrategy::getStrategyId()));
	strategies.push_back(Strategy(Races::Terran, TerranStrategy::getStrategyId()));
	strategies.push_back(Strategy(Races::Zerg, ZergStrategy::getStrategyId()));
...
Commander* StrategySelector::getStrategy()
{
	...
	else
	{
		...
		if (Broodwar->self()->getRace().getID() == Races::Terran.getID()) currentStrategyId = "TerranStrategy";
		if (Broodwar->self()->getRace().getID() == Races::Protoss.getID()) currentStrategyId = "ProtossStrategy";
		if (Broodwar->self()->getRace().getID() == Races::Zerg.getID()) currentStrategyId = "ZergStrategy";
	}

	//Get Commander for strategy
	if (currentStrategyId == "TerranStrategy") return new TerranStrategy();
	if (currentStrategyId == "ProtossStrategy") return new ProtossStrategy();
	if (currentStrategyId == "ZergStrategy") return new ZergStrategy();
