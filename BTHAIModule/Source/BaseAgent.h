...
public:
	...
	/** Checks if there are any enemy units within seekrange. */
	...
	bool doScannerSweep(TilePosition pos);
	 ...
	 * how the agent moves. */
	virtual bool getDefensive();

	/** Adds a new trail entry to a PF pheromone trail. */
	void addTrailPosition(WalkTile wt);

	/** Returns the PF pheromone trail. */
	vector<WalkTile> getTrail();
